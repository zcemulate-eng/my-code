...
export default function CompanyBarChart() {
  ...
  const [bubbleTreeData, setBubbleTreeData] = useState<any>(null); // 👉 新增：用于存放后端返回的树状数据
  ...
  // 3. 核心：获取并分发图表数据 (调用统一的 Analytics 接口)
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const res = await getAnalyticsData(dimension, filters);
      if (res.success) {
        // 分发给 Bar Chart
        setChartData({ labels: res.barChart.labels || [], data: res.barChart.data || [] });
        // 分发给 Bubble Chart
        setBubbleTreeData(res.bubbleChart);
      }
      setLoading(false);
    };

    const timer = setTimeout(fetchData, 500); // 500ms 防抖
    return () => clearTimeout(timer);
  }, [dimension, filters]); // 👉 移除了 tabValue 依赖，一次请求，两图共享
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 3, boxShadow: 3, border: '1px solid rgba(0,0,0,0.02)' }}>
      ...
      <Collapse in={showFilters}>
        <Box sx={{ p: 3, mb: 3, bgcolor: '#fafafa', borderRadius: 2, border: '1px dashed rgba(141,110,99,0.2)' }}>
          <Grid container spacing={2}>
            {/* 👇 修复点：全部改为 size={{ xs: 12, md: 4 }} */}
            <Grid size={{ xs: 12, md: 4 }}>
              <FormControl fullWidth size="small">
                <InputLabel id="filter-level-label">包含等级</InputLabel>
                {/* @ts-expect-error: MUI Select React 19 compatibility */}
                <Select
                  ...
                  renderValue={(selected: number[]) => (
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {selected.map((value) => (
                        <Chip key={value} label={`Level ${value}`} size="small" sx={{ bgcolor: earthColors.light, color: earthColors.dark, fontWeight: 'bold' }} />
                      ))}
                    </Box>
                  )}
                >
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <FormControl fullWidth size="small">
                <InputLabel id="filter-country-label">包含国家</InputLabel>
                {/* @ts-expect-error: MUI Select React 19 compatibility */}
                <Select
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <FormControl fullWidth size="small">
                <InputLabel id="filter-city-label">包含城市</InputLabel>
                {/* @ts-expect-error: MUI Select React 19 compatibility */}
                <Select
      ...
      <Box sx={{ position: 'relative' }}>
        ...
        {/* Tab 1: Bubble Chart (传入后端树状数据) */}
        {tabValue === 1 && (
          <CompanyBubbleChart treeData={bubbleTreeData} />
        )}
      </Box>
    </Paper>
  );
